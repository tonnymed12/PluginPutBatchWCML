sap.ui.define([
    ...
    return PluginViewController.extend("serviacero.custom.plugins.zpluginPutBatchWCML.zpluginPutBatchWCML.controller.MainView", {
        Commons: Commons,
        ApiPaths: ApiPaths,
        formatter: formatter,
        ...
        onGetCustomValues: function () {
            ...
            this.ajaxGetRequest(url, oParams, function (oRes) {
                ...
                aSlotsFixed.forEach(function (slot) {
                    ...
                    // Cantidad asignada persistida en el value: MATERIAL!LOTE!CANTIDAD_STOCK!SECUENCIA!CANTIDAD_ASIGNADA
                    const aParts = (slot.value || "").split('!');
                    slot.cantidadAsignada = aParts.length >= 5 ? (aParts[4] || "") : "";
                });
                ...
                if (iSlotTotal === 0) {
                    ...
                    const maxSecuencia = Math.max(...aSlotsFixed
                        ...
                        .map(slot => parseInt(this.formatter.getSecuencia(slot.value) || 0))
                    );
            ...
            var aPromises = aSlotsConValor.map(function (slot) {
                ...
                return new Promise(function (resolve) {
                    this.ajaxPostRequest(urlLote, inParams,
                        function (oRes) {
                            var sNuevaQty = this._formatLoteQty(oRes.outCantidadLote);
                            var sSecuencia = parts.length >= 5 ? parts[3] : (parts[2] || "");
                            slot.loteQty = sNuevaQty;
                            slot.loteUom = oRes.outOUMLote || "";
                            // Reset de CANTIDAD_ASIGNADA a la nueva cantidad de stock disponible
                            slot.cantidadAsignada = sNuevaQty;
                            slot.value = sMaterial + "!" + sLote + "!" + sNuevaQty + "!" + sSecuencia + "!" + sNuevaQty;
        ...
        */
         ...
         * _procesarSlotValidado, onDeleteSlot, onAddQty), la tabla refleje el estado REAL del backend.
        ...
        _refreshSlotsFromBackend: function () {
            ...
            return this.getWorkCenterCustomValues(sParams, oSapApi).then(function (oData) {
                ...
                // Lista dinámica: solo slots con valor (los vacíos de eliminaciones se ignoran)
                ...
                // Restaurar loteQty/loteUom desde el modelo anterior y reconstruir cantidadAsignada
                // desde el propio value fresco (MATERIAL!LOTE!CANTIDAD_STOCK!SECUENCIA!CANTIDAD_ASIGNADA)
                aSlotsFixed.forEach(function (slot) {
                    if (slot.value) {
                        var parts = slot.value.split('!');
                        var key = parts.slice(0, 2).join('!').toUpperCase();
                        ...
                        slot.cantidadAsignada = parts.length >= 5 ? (parts[4] || "") : (slot.loteQty || "");
                    } else {
                        slot.loteQty = "";
                        slot.loteUom = "";
                ...
                if (iSlotsConValor === 0) {
                    ...
                    var maxSecuencia = Math.max.apply(null, aSlotsFixed
                        ...
                        .map(function (s) { return parseInt(this.formatter.getSecuencia(s.value) || 0); }, this)
                    );
                    this.iSecuenciaCounter = maxSecuencia;
                }
        ...
        _ejecutarUpdate: function (sCantidadLote, sUomLote, sBarcodeIn) {
            ...
            this._refreshSlotsFromBackend().then(function (oRefresh) {
                ...
                // Tabla dinámica: oRefresh.slots solo contiene slots con valor.
                ...
                // Formato: MATERIAL!LOTE!CANTIDAD_STOCK!SECUENCIA!CANTIDAD_ASIGNADA
                // CANTIDAD_ASIGNADA inicia igual a CANTIDAD_STOCK (todo el lote queda asignado al escanear)
                const sCantidadStock = sCantidadLote || "0.00";
                aItems.push({
                    attribute: sNextAttr,
                    value: sBarcode + "!" + sCantidadStock + "!" + this.iSecuenciaCounter + "!" + sCantidadStock,
                    loteQty: sCantidadLote || "",
                    loteUom: sUomLote || "",
                    cantidadAsignada: sCantidadStock
                });
                ...
                const oSapApi = this.getPublicApiRestDataSourceUri();
                console.log("[WCML scan] payload customValues:", JSON.stringify(aCustomValuesFinal));
                this.setCustomValuesPp({
                    inCustomValues: aCustomValuesFinal,
                    inPlant: oPODParams.PLANT_ID,
                    inWorkCenter: oPODParams.WORK_CENTER,
            ...
            this._refreshSlotsFromBackend().then(function (oRefresh) {
                ...
                aSlots.forEach(function (slot) {
                    ...
                    if (aPartes.length >= 2) {
                        ...
                        slot.value = sMaterial + "!" + sLote + "!" + sCantidadStock + "!" + iNuevaSecuencia + "!" + sCantidadAsignada;
        ...
        /**
         * Actualiza la cantidad asignada (cantidadAsignada) del slot de la fila pulsada.
         * Formato de valor: MATERIAL!LOTE!CANTIDAD_STOCK!SECUENCIA!CANTIDAD_ASIGNADA
         * Valida que la cantidad sea > 0 y no exceda la cantidad disponible del lote (loteQty).
         *
         * FLUJO: leer fila → validar → _refreshSlotsFromBackend() → localizar slot por
         *        material!lote → reconstruir value preservando MATERIAL/LOTE/SECUENCIA →
         *        merge con customValues frescos → POST
         */
        onAddQty: function (oEvent) {
            ...
            this._refreshSlotsFromBackend().then(function (oRefresh) {
                ...
                // Localizar el slot por material!lote en datos frescos
                ...
                var sMaterial = aPartes[0];
                var sLote = aPartes[1];
                var sCantidadStock = aPartes.length >= 5 ? aPartes[2] : (aSlots[iIndex].loteQty || sCantidadFormatted);
                var sSecuencia = aPartes.length >= 5 ? aPartes[3] : (aPartes[2] || "");

                aSlots[iIndex].value = sMaterial + "!" + sLote + "!" + sCantidadFormatted + "!" + sSecuencia + "!" + sCantidadFormatted;
                aSlots[iIndex].cantidadAsignada = sCantidadFormatted;

                var oTableUpd = oView.byId("idSlotTable");
                oTableUpd.setModel(new sap.ui.model.json.JSONModel({ ITEMS: aSlots }));
                this._updateOrderSummaryScannedQty(aSlots);
        ...
        _procesarSlotValidado: function (sCantidadLote, sUomLote) {
            ...
            this._refreshSlotsFromBackend().then(function (oRefresh) {
                ...
                // Formato: MATERIAL!LOTE!CANTIDAD_STOCK!SECUENCIA!CANTIDAD_ASIGNADA
                const sCantidadStock = sCantidadLote || "0.00";
                aSlots[iIndex].value = sBarcode + "!" + sCantidadStock + "!" + this.iSecuenciaCounter + "!" + sCantidadStock;
                aSlots[iIndex].loteQty = sCantidadLote || "";
                aSlots[iIndex].loteUom = sUomLote || "";
                aSlots[iIndex].cantidadAsignada = sCantidadStock;
                ...
                const sMaterialLote = materialLoteEscaneado || "";
                const oSapApi = this.getPublicApiRestDataSourceUri();
                console.log("[WCML scanRow] payload customValues:", JSON.stringify(aCustomValuesFinal));
                this.setCustomValuesPp({
                    inCustomValues: aCustomValuesFinal,
                    inPlant: oPODParams.PLANT_ID,
                    inWorkCenter: oPODParams.WORK_CENTER,
                    ...
                    var sErrMsg = (oErr && oErr.responseJSON && (oErr.responseJSON.message || oErr.responseJSON.displayMessage)) || oBundle.getText("errorActualizar");
                    console.error("[WCML-slotRow] Error al guardar slot:", oErr);
                    sap.m.MessageToast.show(sErrMsg);
                    this._slotContext = null;
                    this._refreshSlotsFromBackend();
                }.bind(this));
        ...
        onPressOpenFragmentList: function (oEvent) {
            ...
            if (!this.byId("batchListDialog")) {
                Fragment.load({
                    id: oView.getId(),
                    name: "serviacero.custom.plugins.zpluginPutBatchWCML.zpluginPutBatchWCML.fragment.batchList",
                    controller: this
